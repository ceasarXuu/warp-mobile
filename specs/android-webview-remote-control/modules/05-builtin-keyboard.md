...
- `TerminalKeyboardMode.Builtin` 与 `TerminalKeyboardMode.SystemIme` 在原生层互斥渲染。
- Builtin 模式清除焦点并隐藏系统 IME，只保留 Astropath 风格的三段内置键盘。
...
- 符号网格：`;`, `&`, `*`, `<`, `>`, `?`, `!`, `(`, `)`, `[`, `]`, `\`。
- 方向网格：Up、Left、Down、Right。
- 右侧符号必须走 printable action 路径，不能绕过 modifier 状态和 printable 日志脱敏规则。
...
- side band 宽度沿用 Astropath：`viewportWidth * 0.42`。
- 锚点 offset 沿用 Astropath：`leftPeek=0`、`center=sideBandWidth`、`rightPeek=sideBandWidth*2`。
- 切换阈值沿用 Astropath：`viewportWidth * 0.18`，不是 side band 宽度。
- 拖动结束按起止 offset 判断方向，并在该方向上选择离 end offset 最近的锚点；长拖动允许直接跨过 center。
- 三段内容必须在 viewport 内裁剪；Android Compose 使用 `clipToBounds()` 和 start-aligned oversized row，避免隐藏 band 漏出或右侧 band 被父约束吞掉。
- 真机日志覆盖 `mobile_keyboard_band_drag_started`、`mobile_keyboard_band_drag_completed`、`mobile_keyboard_band_drag_canceled` 和 `mobile_keyboard_anchor_changed`。
...
- 输入权限变化会立即反映到键盘可用状态。
- 键盘不会遮挡终端最后一行；`mobile_shell_keyboard_layout_measured.gap_px` 在稳定状态必须为 `0`。
